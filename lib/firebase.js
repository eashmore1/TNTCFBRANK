...
// Asking for the ETag costs nothing and is what lets the write below be
// conditional, so two people saving at once can't overwrite each other.
async function read() {
  ...
  return {
    data: await res.json(), // null when the path has never been written
    etag: res.headers.get('etag'),
  };
}

// With an etag, RTDB rejects the write with 412 if the data changed after we
// read it — we report that as false and the caller re-reads and retries.
async function write(doc, etag) {
